...
var validate = function validate(card_no) {
  card_no = card_no.split(' ').join(''); //Luhn's algorithm

  var sum = 0;

  for (var i = card_no.length - 1; i >= 0; i--) {
    if (i % 2) {
      sum += Number(card_no[i]);
    } else {
      var double = Number(card_no[i]) * 2;
